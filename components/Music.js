...
import { Button, Modal, TextInput, Appbar } from 'react-native-paper';
...
const Music = () => {
  ...
  function addGoalHandler(enteredGoalText) {
    setCourseGoals((currentCourseGoals) => [
      ...currentCourseGoals,
      { name: enteredGoalText, key: Math.random.toString() }
    ])
  }
  ...
  const Item = ({ title }) => (
    <View style={styles.item}>
      <Text style={styles.title} onPress={deleteGoalHandler}>{title}</Text>
    </View>
  );
  ...
  return (
    <View style={{ marginBottom: 220 }}>
      <Appbar.Header style={{ backgroundColor: "#8250df", color: "#ffffff" }}>
        ...
        <Appbar.Action icon="plus" color="#ffffff" onPress={startAddGoalHandler} />
